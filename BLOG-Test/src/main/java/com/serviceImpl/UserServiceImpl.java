		...
		try {
			...
			if (authentication.isAuthenticated()) {
				if (myUserDetailsService.getUserDetails().getStatus().equalsIgnoreCase("true")) {
					...
					return new ResponseEntity<String>("{ \"token\" : \""+jwtUtils.generateToken(myUserDetailsService.getUserDetails().getEmail(), myUserDetailsService.getUserDetails().getRole())+"\"}",HttpStatus.OK);
				} else {
